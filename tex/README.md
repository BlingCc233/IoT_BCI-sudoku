...
## 本仓库的项目化用法（IoT\_BCI-sudoku）

- 项目论文入口：`main.tex`
- 原模板示例（保留参考）：`template_original_main.tex`
- 自动生成对比图表/表格（输出到 `generated/`，可直接 `\\input{generated/...}` 插入论文）：

```bash
go run ../cmd/iotbci-texgen -bench ../bench.json -evidence ../evidence_out/evidence.json -out_dir generated
```
