...
\section{设计目标与总体思路}
面向 BCI-IoT 的协议实现需要同时满足四项要求：一是握手阶段具备身份认证与抗重放能力；二是会话阶段提供机密性与完整性；三是在密文条件下仍能降低流量外观可学习性；四是关键状态具备上界，适配资源受限设备。基于此，本文采用认证握手、会话记录层与纯组合数独外观层的分层设计。握手与记录层接口设计参考 TLS/DTLS 与 AEAD 标准化实践\cite{rfc8446,rfc9147,rfc5116}。

本文所设计的协议在通用安全协议栈之外增加面向 BCI-IoT 场景的外观调控与异常处置机制：握手失败进入可疑路径处理；重放缓存采用有界结构；外观层映射构表执行冲突检查。上述机制共同构成安全正确性与可部署性的实现基础。

\section{协议分层与数据通路}
协议部署位置位于应用层与 TCP 之间。数据在发送方向依次经历应用负载封装、会话记录层加密与纯组合数独外观编码；接收方向按逆序解码与解密。其数据通路如图 \ref{fig:c3-stack} 所示。
...
\node[box] (a3) at (0,-3.0) {纯组合数独外观层：4 字节提示映射 + 填充注入};
...
\end{tikzpicture}
\caption{组合数独协议在应用层与 TCP 之间的分层通路：握手、AEAD 记录与外观层协同}
\label{fig:c3-stack}
\end{figure}
...
\end{tikzpicture}
\caption{认证握手三步时序：ClientHello、ServerHello 与 ClientFinish}
...
\centering
\small
\caption{握手消息字段摘要：证书认证、协商位与新鲜性信息}
...
其中 \texttt{flags} 的低位用于协商会话 AEAD 算法；\texttt{bit2} 表示下行是否启用纯模式。本文实验设置固定为纯模式路径，避免模式切换对结论造成混淆。
...
\draw[line] (c2) -- (d1);
\end{tikzpicture}
\caption{可疑流量命中后的 silent/fallback 回落与诱饵转发流程}
\label{fig:c3-fallback-flow}
\end{figure}

\subsection{与对照方案的异常处置能力差异}
表 \ref{tab:c3-fallback-compare} 给出本文实现中各路线的异常处置对照。TLS/DTLS、MQTT/TLS 与 CoAP/DTLS 在标准流程下通常表现为告警、断开或超时重试，不提供携带已读原始字节并转发诱饵的协议级接口；组合数独协议在握手层显式输出可疑连接上下文，用于支持统一回落策略\cite{rfc8446,rfc9147,mqtt311,rfc7252}。

\begin{table}[H]
\centering
\small
\caption{握手异常处置对照：组合数独保留已读字节回落接口}
...
\midrule
组合数独协议 & 返回可疑错误与连接包装，按策略执行 silent/fallback & 是 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{与对照握手路径的实现差异}
为突出工程差异，本文将组合数独协议与三类对照握手路径比较如下：
\begin{itemize}
  \item \textbf{纯 AEAD}：无独立握手；会话材料由预共享密钥直接派生；
  \item \textbf{MQTT/TLS}：安全建立由 TLS 完成，MQTT 层不参与握手密码学验证；
  \item \textbf{CoAP/DTLS}：安全建立由 DTLS 完成，CoAP 层负责报文语义与重传；
  \item \textbf{组合数独协议}：握手状态机与外观层在同一协议内协同，握手成功后再切换会话记录层。
\end{itemize}

因此，组合数独协议的安全性增益主要体现为握手认证、会话加密、外观调控与异常终止机制的协同完整性。

\section{纯组合数独外观层实现}
\subsection{映射构表}
外观层先枚举 4$\times$4 数独解空间，再基于会话种子重排并抽取 256 个网格与字节值对应。关于“4$\times$4 解空间为 288”的来源，OEIS 的 A109252 与 A107739 两个序列在对应项上给出一致结果，即 4$\times$4 数独完备解空间规模为 288\cite{oeisA109252,oeisA107739}。同时，A109252 还给出在 4$\times$4 网格上取 4 个提示且保持唯一解时的题面数量为 25728，这说明在“小规模唯一解题面”约束下仍存在较大的组合空间，可用于支持映射重排并提升攻击者仅凭题面模式实施稳定侧写的难度\cite{oeisA109252}。每个字节维护多个候选提示组；构表阶段执行跨字节解码键冲突检测，若冲突则拒绝构表。该步骤保证解码映射单值性。

\subsection{编码与解码}
编码阶段对每个字节输出 4 个提示字节，并在多个检查点按概率插入填充字节。解码阶段仅提取提示字节，按 4 个一组拼接解码键并查表还原。由于纯模式下最外层始终是提示字节流，协议外观可控且解码路径稳定。

\subsection{纯模式路径图解与可逆性验证}
图~\ref{fig:c3-full-pipeline} 给出纯模式路径从发送到接收的完整闭环。该闭环与编码解码一致性验证实验相对应：发送端执行映射与填充，接收端执行筛选与还原，并以字节级一致性判定可逆性。
...
\node[box] (p2) at (5.8,0) {组合数独映射\\1B $\rightarrow$ 4 提示字节};
...
\end{tikzpicture}
\caption{纯模式编解码闭环与可逆性验证：映射、填充、查表恢复}
\label{fig:c3-full-pipeline}
\end{figure}

\begin{table}[H]
\centering
\small
\caption{纯模式路径可逆性验证：冲突为零且往返成功率 100\%}
\label{tab:c3-full-verify-map}
\begin{tabular}{p{2.8cm}p{4.0cm}p{3.9cm}p{2.2cm}}
\toprule
阶段 & 功能环节 & 验证操作 & 判定条件 \\
\midrule
映射构表 & 组合数独映射构表与冲突检测模块 &
...
纯组合数独外观层支持内置 ASCII/高熵布局与自定义字节模板（模式模板）。自定义模板约束为 8 bit 上 ``2 个 x + 4 个 p + 2 个 v''，其中 $x$ 为冗余判别位、$p$ 为位置位、$v$ 为取值位；只有同时满足提示掩码/提示取值条件的字节才会被解码器视为提示字节。该机制使协议可在“可打印性优先”和“高混淆优先”之间切换。
...
\centering
\small
\caption{ASCII/Entropy/Custom 模式的提示掩码、填充池与汉明重特点}
...
从可计算复杂性视角看，数独映射可视为约束满足/可满足性编码问题\cite{lynce2006satgrid}，这为后续构表冲突检测与可逆性证明提供了理论支点。

\subsection{6bit分包组合数独编码路径}
除纯模式路径外，本文还设计了 6bit分包组合数独。其基本过程是把明文字节流按 6-bit 分组，再映射为符合组合数独字节布局的外观字节。对连续 3 字节输入 $(b_1,b_2,b_3)$，可得到 4 个 6-bit 分组：
\[
\begin{aligned}
g_1 &= (b_1\!\gg\!2),\\
...
\end{aligned}
\]
该路径保留了组合数独字节外观风格（可打印布局与自定义模板仍可生效），但不再执行“每原始字节 $\rightarrow$ 4 提示字节 $\rightarrow$ 数独网格”的离散映射过程，因此混淆强度低于纯模式路径。其工程取舍为：
\begin{itemize}
  \item 优点：线长与 RTT 通常优于纯模式，带宽利用率更高；
  \item 缺点：外观随机性下降，对高级侧写模型的抑制能力弱于纯模式；
  \item 适用：链路带宽更紧、但仍希望保持组合数独外观风格的场景。
\end{itemize}
...
\node[box] (o) at (4.0,-4.0) {4 个 6-bit 组\\$\rightarrow$ 组合数独外观字节流};
...
\draw[line] (g4) -- (o);
\end{tikzpicture}
\caption{6bit分包组合数独的 3B$\rightarrow$4 组分解示意}
\label{fig:c3-segment-bitflow}
\end{figure}

\begin{table}[H]
\centering
\small
\caption{纯模式与分包模式的编码颗粒、混淆强度与带宽开销对照}
\label{tab:c3-mode-mechanism}
...
\subsection{核心算法伪代码}
下文给出握手状态机、纯模式编码与异常回落三类机制的形式化伪代码。

\begin{algorithm}[H]
\small
\caption{服务端握手校验：探测、认证、重放检查后切换会话}
...
\begin{algorithm}[H]
\small
\caption{纯模式写路径：按阈值插入填充并输出 4 个提示字节}
\label{alg:c3-full-write}
...
\begin{algorithm}[H]
\small
\caption{可疑流量回落策略：silent 丢弃或转发至诱饵连接}
...
\section{本章小结}
本章完成了协议从机制到实现的系统化设计：三次握手完成认证与密钥协商，记录层提供会话机密性与完整性，纯组合数独外观层承担侧写对抗与外观可控任务，重放缓存与可疑路径机制用于约束异常输入范围。上述实现构成下一章实验验证的技术基础。
