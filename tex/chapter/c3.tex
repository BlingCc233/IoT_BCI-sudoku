...
\section{设计目标与总体思路}
面向 BCI-IoT 的协议实现需要同时满足四项要求：一是握手阶段具备身份认证与抗重放能力；二是会话阶段提供机密性与完整性；三是在密文条件下仍能降低流量外观可学习性；四是关键状态具备上界，适配资源受限设备。基于此，本文采用“认证握手 + 会话记录层 + Sudoku-pure 外观层”的分层设计。握手与记录层接口设计参考 TLS/DTLS 与 AEAD 标准化实践\cite{rfc8446,rfc9147,rfc5116}。

与直接复用通用协议栈不同，本文实现强调协议语义与工程边界的一体化：握手失败进入可疑路径处理；重放缓存采用有界结构；外观层映射构表执行冲突检查。上述机制共同构成“安全正确性 + 可部署性”的实现基础。
...
上述性质由签名验证、摘要绑定、确认码与重放缓存共同提供，其成立条件与“外观编码可破解性”假设无关。这与 TLS/DTLS 的设计思想一致，即握手安全由身份认证、密钥协商与 transcript 绑定保障\cite{rfc8446,rfc9147}。
...
为避免“异常即断开”带来的可区分行为，本文在可疑路径上实现了回落机制。其接口契约为：握手阶段一旦命中可疑条件，服务端返回
\[
...
\]
其中 $B_{\mathrm{rec}}$ 为协议栈已读且缓存的原始线上字节，$B_{\mathrm{live}}$ 为触发后继续到达的剩余字节。该设计可保证诱饵侧看到的输入连续完整，避免因“前缀丢失”暴露额外判别信号。
...
表 \ref{tab:c3-fallback-compare} 给出本文实现中各路线的异常处置对照。可以看到，TLS/DTLS、MQTT/TLS 与 CoAP/DTLS 在标准流程下通常表现为告警/断开或超时重试，不提供“携带已读原始字节并转发诱饵”的协议级接口；Sudoku 在握手层显式输出可疑连接上下文，便于上层执行统一回落策略\cite{rfc8446,rfc9147,mqtt311,rfc7252}。
...
因此，Sudoku 的安全性增益主要不在单一算法选择，而在“握手认证 + 会话加密 + 外观调控 + 异常终止”的组合流程完整性。

\section{Sudoku-pure 外观层实现}
\subsection{映射构表}
外观层先枚举 4$\times$4 Sudoku 解空间，再基于会话种子重排并抽取 256 个网格与字节值对应。每个字节维护多个候选提示组；构表阶段执行跨字节解码键冲突检测，若冲突则拒绝构表。该步骤保证解码映射单值性。
...
图~\ref{fig:c3-pure-pipeline} 给出 pure 路径从发送到接收的完整闭环。该闭环与编码解码一致性验证实验相对应：发送端执行“映射 + 填充”，接收端执行“筛选 + 还原”，并以字节级一致性判定可逆性。
...
\subsection{核心算法伪代码}
为保证“机制描述 - 实现逻辑 - 实验证据”三者一致，本文将握手状态机、pure 编码与异常回落三条关键路径抽象为伪代码。
...
建立到诱饵地址的连接 $dst$\;
从 $wrapper$ 读取“已读+缓存”字节前缀并写入 $dst$\;
启动双向转发：$raw \rightarrow dst$ 与 $dst \rightarrow raw$\;
\end{algorithm}

\subsection{伪代码与仓库实现映射}
表 \ref{tab:c3-code-map} 给出伪代码与仓库函数的一一对应关系，便于读者从论文直接定位到实现与测试入口。
...
本文选用 Go 作为协议主实现语言，核心原因有三点：其一，Go 标准库提供稳定的网络与密码学接口，便于将握手、记录层与外观层在同一代码基中组织；其二，协程与阻塞 I/O 组合适合连接态协议与并发评测场景；其三，静态编译与跨平台构建便于在本地与跨区域服务器重复部署\cite{golangOfficial,goPackagesStd}。在此基础上，本文将“协议实现、证据采集、论文出图”分别落在最合适工具上，形成可复核流水线（表 \ref{tab:c3-toolchain}）。
...
工具 & 主要用途 & 选型理由 \\
\midrule
Go & 协议主实现、基线对照、自动化 bench & 网络/并发/密码库完整，工程可部署性强\cite{golangOfficial,goPackagesStd} \\
\midrule
Python & \path{evidence_steps} 复现实验脚本与数据整形 & 数据处理与脚本编排效率高，便于快速复核\cite{pythonDocs} \\
\midrule
libpcap\\Wireshark & 抓包、会话复核、字段核对 & 行业通用抓包分析链路，便于第三方复核\cite{wiresharkDocs} \\
\midrule
GitHub\\Actions & 压测与回归流程自动执行 & 保证结果可重复并降低人工操作偏差 \\
\midrule
LaTeX +\\PGFPlots & 论文排版与图表绘制 & 学术写作可控性高，图表与数据可联动\cite{latexProject,pgfplotsManual} \\
\bottomrule
\end{tabular}
\end{table}

\section{本章小结}
本章给出了协议从机制到代码落地的完整设计：三次握手完成认证与密钥协商，记录层提供会话机密性与完整性，Sudoku-pure 外观层承担侧写对抗与外观可控任务，重放缓存与可疑路径机制保证异常输入下的安全边界。上述设计为下一章实验验证提供实现基础。
